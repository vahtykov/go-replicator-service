# Go Replicator Service

Сервис real-time репликации данных PostgreSQL между географически распределенными контурами через Kafka.

## Документация

📋 **[ARCHITECTURE_OVERVIEW.md](ARCHITECTURE_OVERVIEW.md)** - Краткая архитектура решения ⭐ **НАЧНИТЕ ОТСЮДА**
- Общая схема системы
- Компоненты: ReplicatorPublisher, ReplicatorConsumer
- Защита от петли репликации (подробно!)
- Достоинства и ограничения

📚 **[HYBRID_SOLUTION.md](HYBRID_SOLUTION.md)** - Детальное техническое описание
- Примеры кода на Go и SQL
- Конфигурация
- План реализации

## Ключевые особенности

✅ **Real-time репликация** (1-3 сек задержка)  
✅ **Не требует изменений в приложении** (триггеры PostgreSQL)  
✅ **Защита от потери данных** (replication_log)  
✅ **Идемпотентность** (processed_events)  
✅ **Разрешение конфликтов** (версионирование)  
✅ **Защита от петли репликации** (session_replication_role)  

## Архитектура

```
Приложение → PostgreSQL [Триггеры] → replication_log
                                           ↓
                                   ReplicatorPublisher
                                           ↓
                                        Kafka
                                           ↓
                                   ReplicatorConsumer
                                           ↓
                              PostgreSQL (другой контур)
```

## Быстрый старт

### 1. Настройка PostgreSQL ⚡

```bash
# Один скрипт для полной настройки!
psql -U your_user -d your_database -f sql/00_master_setup.sql
```

Затем настройте свои таблицы:

```sql
-- Одна команда на таблицу (добавляет поля + создает триггеры)
SELECT setup_table_for_replication('users');
SELECT setup_table_for_replication('orders');
SELECT setup_table_for_replication('products');
```

✅ **Триггер автоматически:**
- Захватывает все INSERT/UPDATE/DELETE
- Записывает в `replication_queue`
- Защищен от петли репликации (через `session_replication_role`)

📚 Подробная документация: [sql/README.md](sql/README.md)

### 2. Запустить сервисы

```bash
# ReplicatorPublisher
go run cmd/publisher/main.go

# ReplicatorConsumer
go run cmd/consumer/main.go
```

## Структура проекта

```
go-replicator-service/
├── sql/                          # SQL скрипты для PostgreSQL
│   ├── 00_master_setup.sql      # Автоматическая установка
│   ├── 01_create_tables.sql     # Служебные таблицы
│   ├── 02_create_replication_trigger.sql  # Функции триггеров
│   ├── 03_setup_example.sql     # Примеры и тесты
│   ├── 04_migrate_existing_tables.sql     # Миграция существующих таблиц
│   ├── README.md                # Документация SQL
│   ├── CHEATSHEET.md            # Быстрая справка
│   └── FILES.md                 # Описание файлов
│
├── cmd/                         # Исполняемые файлы
│   ├── publisher/               # ReplicatorPublisher (скоро)
│   └── consumer/                # ReplicatorConsumer (скоро)
│
├── internal/                    # Внутренние пакеты (скоро)
│   ├── config/
│   ├── kafka/
│   ├── database/
│   └── processor/
│
├── ARCHITECTURE_OVERVIEW.md     # Архитектура (⭐ начните отсюда)
├── HYBRID_SOLUTION.md           # Детальное описание
└── README.md                    # Этот файл
```

## Требования

- PostgreSQL 10+
- Kafka (георезервированная)
- Go 1.21+

## Лицензия

MIT

